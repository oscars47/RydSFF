"""
plotting single qubit bloch vector evolution
"""
...
from QuEraToolbox.hamiltonian import drive_main, get_J_arr, H_int
...
def draw_bloch_sphere(ax, fontsize=20, show_theta_phi=True):
    ...
    if show_theta_phi:
        ...
        phi_arc = np.linspace(np.pi/2, np.pi, 20)  
        z_level = 0.7 
        radius_at_z = np.sqrt(1 - z_level**2)  
...
def evolve_state(evals, evecs, psi0, t_ls, q_idx=None):
    ...
    overlaps = np.array([float(overlap) for overlap in overlaps])  
    
    # vectorized phase computation 
    phases_matrix = np.exp(-1j * np.outer(evals, t_ls))  
    
    coeffs_matrix = overlaps[:, None] * phases_matrix  
    ...
    if q_idx is not None:
        ...
        # fully vectorized reconstruction 
        psi_t_ls = [sum(evecs[i] * coeffs_matrix[i, t_idx] for i in range(len(evals))) 
                    for t_idx in range(len(t_ls))]
        return psi_t_ls

def fig1_chain(t_ls, Delta_local_ls=[ -0.5*5.42, -10*5.42,], Delta_mean=0.5*5.42, h_ls=[
            0.24996654759766535,
            0.09286441773368292,
            0.03781977537875725,
            0.5848043295638031,
    ...
    fig_path = os.path.join(dir_root, "results", f"fig1_chain_{max(t_ls)}.pdf")
    plt.savefig(fig_path)

from fig_supplemental_signatures import repeat_quantities_general, all_quantites_one_time
def make_fig1_entropy_only(N, t_ls, Delta_local_ls, Delta_mean_ls, n_repeats, dir_root, fontsize=40, alpha=0.8, J=5.42, **kwargs):
    ...
    mpl.rcParams.update({'font.size': fontsize, 'text.latex.preamble':r'\usepackage{xcolor}'})
    plt.rc('text', usetex=True)
    mpl.rc('text.latex', preamble=r"""
    \usepackage{amsmath}
    \usepackage{newtxtext,newtxmath}
    ...
    for i, Delta_mean in enumerate(Delta_mean_ls):
        for j, Delta_local in enumerate(Delta_local_ls):
            ...
            all_results = repeat_quantities_general(N, t_ls, H_int_, Delta_local, Delta_mean, n_repeats, func = all_quantites_one_time, fileprefix='fig1',dir_root=dir_root, add_renyi2=True, **kwargs)

            
            vn_time_all = np.array([res['vn_time'] for res in all_results])
            vn_time = np.mean(vn_time_all, axis=0)

            renyi2_time_all = np.array([res['renyi2_time'] for res in all_results])
            renyi2_time = np.mean(renyi2_time_all, axis=0)
            
    
            axs.plot(
                t_ls, vn_time,
                linestyle='-', alpha=alpha, color=colors[j], linewidth=6, label=fr'$S_{{1, A}}, \, \Delta_{{\mathrm{{local}}}}={Delta_local/J:.3g} J$'
            )
            axs.plot(
                t_ls, renyi2_time,
    ...
    # axs.set_ylabel(
    # r'$S_{1,\,A}(t) \, \textcolor{gray}{\ S_{2,\,A}(t)}$',
    # fontsize=1.3*fontsize)
    axs.set_ylabel('')  # clear default
    axs.annotate(
        r'$S_{1,\,A}(t),$',
        xy=(-0.11, 0.35), xycoords='axes fraction',
